<template>
  <el-tabs v-model="activeTab">
    <!-- ===== 页签1：入库单列表（审核流） ===== -->
    <el-tab-pane label="入库单管理" name="list">
      <el-card class="mb">
        <el-form inline>
    ...
    <!-- ===== 页签2：新建入库单（草稿，不动库存） ===== -->
    <el-tab-pane label="新建入库单" name="create">
      <el-card>
        <h3>创建入库单（草稿）</h3>
...
import {
  getStockInPage, submitStockIn, withdrawStockIn,
  approveStockIn, rejectStockIn, postStockIn, createStockIn, reverseStockIn
} from '../api/stock'
...
function canSubmit(row) { return row.status === 0 }                       // 草稿→待审（所有人）
function canApprove(row) { return canGoToAudit() && row.status === 1 }    // 待审→已审
function canReject(row) { return canGoToAudit() && row.status === 1 }     // 待审→草稿
function canWithdraw(row) { return row.status === 1 }                     // 待审→草稿（全员）
function canPost(row) { return canGoToAudit() && row.status === 2 }       // 已审→已过账
...
const ACTION_MSG = {
  ...
  withdraw: '确认撤回？将退回草稿。',
  post: '确认过账？过账后将正式更新库存并记录流水，且不可撤销。'
}
const ACTION_API = {
  submit: submitStockIn, approve: approveStockIn,
  reject: rejectStockIn, withdraw: withdrawStockIn, post: postStockIn
}
...
async function doReverse(row) {
  ...
  try {
    const r = await ElMessageBox.prompt(
      `对入库单 ${row.orderNo} 执行红冲（冲销），将回滚库存并生成红冲单。\n请输入红冲原因：`,
      '红冲确认',
      { confirmButtonText: '确认红冲', cancelButtonText: '取消', inputPlaceholder: '必填，如：录错数量/录错供应商', inputValidator: v => (v && v.trim() ? true : '红冲原因不能为空') }
    )
    remark = r.value
  } catch (e) { return }
  try {
    await reverseStockIn(row.id, remark)
...
async function handleCreate() {
  ...
  saving.value = true
  try {
    await createStockIn({ ...form, items: valid.map(i => ({ productId: i.productId, quantity: i.quantity, unitPrice: i.unitPrice })) })
    ElMessage.success('入库草稿已创建')
